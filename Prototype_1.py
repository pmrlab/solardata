...
# GOTO WITH RETRY
# =========================
def goto_with_retry(page, url, retries=5, wait_between=15):
    """Navigate to URL, retrying on network errors."""
    for attempt in range(1, retries + 1):
        try:
            ...
            if attempt < retries:
                ...
                raise RuntimeError(f"Failed to load {url} after {retries} attempts.") from e
...
with sync_playwright() as p:
    ...
    # 1️⃣ LOGIN
    goto_with_retry(page, "https://web3.isolarcloud.com.hk/#/login")
    ...
    # =========================
    # SCROLL DOWN AFTER MULTIPATH 5
    # =========================
    page.mouse.wheel(0, 3000)
    page.wait_for_timeout(2000)

    # =========================
    # DOWNLOAD PRABHA PLANTS
    # =========================
    for plant in ["Prabha 01", "Prabha 02"]:
        page.get_by_role("img").nth(2).click()
        page.get_by_text(plant).click()
        page.wait_for_timeout(2000)
